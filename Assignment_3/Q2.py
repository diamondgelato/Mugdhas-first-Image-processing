# Using loops, fill an image with randomly coloured, equal sized squares in the following manner:
# i) left to right for even numbered row.
# ii) right to left for odd numbered row.
# iii) Introduce a delay of 0.5 second, so that the process of filling of the image is visible.
...
for y in range (1, 8):
    ...
    for x in range (start, end, step):
        ...
        cv2.imshow ('coloured box fills', img)
        cv2.waitKey(500)
